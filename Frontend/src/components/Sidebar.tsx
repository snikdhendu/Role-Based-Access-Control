...
import { Users, Shield, CreditCard, Home, MoreVertical, User2, LogOut } from 'lucide-react';
      ...
      >
        <div className="flex h-full w-64 flex-col bg-white border-r border-gray-200 shadow-lg">
          <Link to="/" className="flex h-16 items-center justify-center border-b border-gray-200">
            ...
            <span className="ml-2 text-xl font-bold text-gray-900">RBAC {user?.role=='admin' ? (<>Admin</>):(<>User</>)}</span>
          </Link>
          <nav className="flex-1 space-y-1 px-2 py-4">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon
                    className={`${isActive ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-600'
